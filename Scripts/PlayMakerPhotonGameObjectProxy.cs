...
public class PlayMakerPhotonGameObjectProxy : MonoBehaviourPunCallbacks {
	...
	/// </param>
	public void PhotonRpcSendFsmEvent(RpcTarget target,string globalEventName)
	{
		if (LogMessageInfo)
		{
			Debug.Log("RPC to send global Fsm Event:"+globalEventName+" to self, with network target:"+target);	
		}
	...
	/// </param>
	public void PhotonRpcSendFsmEventWithString(RpcTarget target,string globalEventName,string stringData)
	{
		if (LogMessageInfo)
		{
			Debug.Log("RPC to send string:"+stringData+"  with global Fsm Event:"+globalEventName+" to self, with network target:"+target);	
		}
	...
	/// the ability for FsmEvent to include data.
	/// </param>
	public void PhotonRpcSendFsmEventWithData(RpcTarget target,string globalEventName,FsmVar[] data)
	{
		if (LogMessageInfo)
		{
			Debug.Log("RPC to send "+data.Length+" data with global Fsm Event:"+globalEventName+" to self, with network target:"+target);	
		}
